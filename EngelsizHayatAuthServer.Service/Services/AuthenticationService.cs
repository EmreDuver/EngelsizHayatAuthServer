    ...
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<Response<TokenDto>> CreateTokenAsync(LoginDto loginDto)
        {
            ...
            if (!user.EmailConfirmed)
                return Response<TokenDto>.Fail("Please confirm your email.  ", 400, true);
        ...
        public async Task<Response<TokenDto>> CreateTokenByRefreshToken(string refreshToken)
        {
            ...
            {
                return Response<TokenDto>.Fail("Refresh token not found!", 410, true);
            }
            var user = await _userManager.FindByIdAsync(existRefreshToken.UserId);
            if (user == null)
            {
                return Response<TokenDto>.Fail("User Id not found!", 410, true);
            }
        ...
        public async Task<Response<NoDataDto>> RevokeRefreshToken(string refreshToken)
        {
            ...
            if (existRefreshToken == null)
            {
                return Response<NoDataDto>.Fail("Refresh token not found!", 410, true);
            }
